...
func TestNewHiraganaWord(t *testing.T) {
	word := "あいうえおabcdefg"
	got, err := entity.NewHiraganaWord(word)
	e := &entity.ErrNotHiragana{word}
...
func TestHiraganaWordString(t *testing.T) {
	...
	w, err := entity.NewHiraganaWord(str)
	assert.Nilf(t, err, "unexpected error: %v", err)
	got := w.String()
...
func TestHiraganaWordValidateChain(t *testing.T) {
	s1 := "ごりら"
	s2 := "らいおん"
	w1, err := entity.NewHiraganaWord(s1)
	assert.Nilf(t, err, "unexpected error: %v", err)
	w2, err := entity.NewHiraganaWord(s2)
	assert.Nilf(t, err, "unexpected error: %v", err)
